...
def paralFunc(v1,v2):
    #print(i)
    return wignerRotation(v1,v2)
    #print(v1,v2)
    #print(gc.garbage)
    #todo =  wignerRotation(v1,v2)
    #return todo
    #print(todo)
...
if __name__ == '__main__':
    ...
    print("TOTAL nSTART and nEND:",nStart, nEnd)
    
    """
    t0 = time.time()
    with closing(Pool(3)) as p:
        temp3 = (p.starmap(paralFunc, [(clusters[i], clusters[j],i) for i in range(nStart,nEnd) for j in range(0,i+1)] ))
#print("Process is alive", multiprocessing.current_process().is_alive())
    ...
    t0 = time.time()
    start = nStart
    end = nStart + int((nEnd-nStart)/pDivs)
    print(start,end)
    print(start,end)
    print(start,end)
    print(start,end)
    ...
    for i in range(0,length):#fill up the respective arrays to store
        varTemp[i] = temp[i][0]
        relTemp[i] = temp[i][1]
#print("Process is alive", multiprocessing.current_process().is_alive())
    ...
    for i in range(nStart, nEnd):
        for j in range(0, i+1):
            p = Process(target = paralFunc, args=(clusters[i],clusters[j]))
            p.start()
            p.join()
    t1 = time.time()
    print("TIme Taken:", t1-t0)
