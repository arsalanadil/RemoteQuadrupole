...
def I(l,delR,z1,z2):
    ...
    func = lambda k: (k**2 * powerSpec(k) * (tf.delta(k,z1)[0] * tf.delta(k,z2)[0])  * bessel(l,(k*deltR)))
    ...
    temp = quad(func,0,300, limit=50, epsrel = 1e-5,epsabs = 1e-5)
...
def CartToSpher(position):
    ...
    phi = np.arctan2(np.sqrt(y**2 + x**2),z)
...
def aBarCor(delR,Integrand):
    ...
    for mj in range(-2,3):
        for mk in range(-2,3):
            ...
            #print("unrotated var using VarMatrixGeneralized:", var)
...
def wignerRotation(V1,V2):
    ...
    for i in range(0,len(Integrand),2):
        Integrand[i] = I(i,delR,z1,z2)[0]#toDo: pass in delR rather than Vr1 and Vr2;
...
t0 = time.time()
temp=wignerRotation(v1,v2)[0][0][0]
t1 = time.time()
print(temp)
...
v4 = np.array([1,0,0])
t0 = time.time()
temp=wignerRotation(v0,v4)[0][0][0]
t1 = time.time()
print(temp)
print("Time this process took:", t1-t0)
"""
