...
"""
-----------------
The following is to test Seto and Pierpaoli fig. 2:
"""
n = 11
l = 3
zmax = 2.1
...
i=0
zlist = np.arange(0.1,zmax,zmax/n)

for z in zlist:
    C2[i] =  KIntegral(z,z,l) 
    i+=1

plt.plot(zlist,7/5*C2/C20)
...
for i in philist:
    ...
    #temp[j] = ((cov + np.conjugate(cov))/2)**2 + (1j*(cov - np.conjugate(cov))/2)**2
    j+=1
