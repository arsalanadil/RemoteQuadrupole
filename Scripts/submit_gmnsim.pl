...
system("rm /home/aa9pb/adil/test/results/*");
system("rm /home/aa9pb/adil/test/recresults/*");
...
system("act -g nodes -x physics05 rm -r /tmp/aa9pb/adil/ndesim");
system("act -g nodes -x physics05 mkdir -p /tmp/aa9pb/adil/ndesim");
#
...
# build the name of log file for this case.
    ...
    system("sed '26s%source_dir=%source_dir=/home/aa9pb/adil%' step1.sh > step2.sh");
    system("sed '27s%remote_dir=%remote_dir=/home/adil/ndesim/$counter%' step2.sh > run_sqrt.sh");
    ...
    system("cp run_sqrt.sh inputScripts/run_sqrt$counter.sh");
#
...
#
    ...
    system("qsub -l nodes=2:ppn=2 -l walltime=32:00:00 -l mem=8GB -j oe -o $logfile inputScripts/run_sqrt$counter.sh \n");
    if ($counter <=  ($njobs-1)) {
